...
export function QnAListPage({
  ...
  const handleRowClick = (item) => {
    const isSecretMasked = item.secret_yn === "Y" && !item.isMine;
    if (isSecretMasked) {
                  ...
                  {pagedItems.length === 0 ? (
                    ...
                    pagedItems.map((item) => {
                      ...
                      return (
                        <tr
                          key={item.id}
                          className={`clickable-qna-row ${item.isMine ? "my-post-row" : ""} ${isSecretMasked ? "secret-post-row" : ""}`}
                        ...
                        >
                          ...
                          <td className="td-title">
                            {isSecretMasked ? (
                              ...
                              <>
                                {item.isMine && <span className="badge-my-post">내 글</span>}
                                {item.secret_yn === "Y" && (
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#887d74" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: "middle", marginRight: "5px" }}>
                                    <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                                    <path d="M7 11V7a5 5 0 0 1 10 0v4" />
