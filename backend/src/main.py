...
# 2. 업로드 및 정적 이미지 파일 서빙 디렉터리 준비 및 마운트
UPLOAD_DIR = Path("uploads")
UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
app.mount("/uploads", StaticFiles(directory="uploads"), name="uploads")

# 상품/배너/아이콘 정적 이미지 디렉터리 마운트 (/images 주소로 접근 가능)
STATIC_IMAGES_DIR = Path("images")
if not STATIC_IMAGES_DIR.exists():
    PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
    STATIC_IMAGES_DIR = PROJECT_ROOT / "an" / "react-baseason-frontend" / "public" / "images"
