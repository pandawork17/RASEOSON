...
# =========================================================
# 5. DB 동적 테이블 카탈로그 및 데이터 탐색기 (song/frontend)
# =========================================================
@router.get("/api/admin/tables", summary="DB 전체 테이블 목록 및 요약 정보 동적 조회")
def get_admin_tables(db: Session = Depends(get_db)):
    """
    MySQL 데이터베이스(shopdb3jo)에 존재하는 모든 테이블 목록을 동적으로 조회합니다.
    테이블명, 총 레코드 수, 기본키(PK) 컬럼명을 반환합니다.
    하드코딩 없이 실제 DB 스키마 및 레코드가 실시간 반영됩니다.
    """
    bind = db.get_bind()
    insp = inspect(bind)
...
@router.get("/api/admin/tables/{table_name}", summary="DB 테이블 메타데이터 및 전체 행 데이터 동적 조회")
def get_admin_table_detail(table_name: str, db: Session = Depends(get_db)):
    """
    지정된 테이블의 실제 컬럼 스키마(이름, 타입, PK 등)와 데이터를 동적으로 조회합니다.
    - 데이터가 0건이어도 실제 DB 스키마에 정의된 모든 컬럼 정보가 반환됩니다.
    - 모든 컬럼 값은 JSON 직렬화 가능한 형태로 안전하게 변환됩니다.
    """
    bind = db.get_bind()
    insp = inspect(bind)
    ...
    if table_name not in existing_tables:
        raise HTTPException(status_code=404, detail=f"테이블 '{table_name}'을(를) DB에서 찾을 수 없습니다.")
    ...
    # 데이터 조회
    try:
        data_res = db.execute(text(f"SELECT * FROM `{table_name}` LIMIT 1000"))
        rows_dict = [dict(row) for row in data_res.mappings().all()]
        ...
        for row in rows_dict:
            ...
            for col_name in column_names:
                serialized_row[col_name] = serialize_db_value(row.get(col_name))
            serialized_rows.append(serialized_row)

        total_count = db.execute(text(f"SELECT COUNT(*) FROM `{table_name}`")).scalar() or len(serialized_rows)
        ...
        raise HTTPException(status_code=500, detail=f"테이블 조회 중 오류 발생: {str(e)}")


# =========================================================
# 6. 레거시/탐색용 원천 테이블 조회 API 지원 (하위 호환성)
# =========================================================
RAW_TABLES = [
