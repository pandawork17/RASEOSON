-- =============================================================================
-- [BASEASON_shopdb3jo_최종복구.sql]
-- 베이스시즌(BASEASON) 쇼핑몰 맞춤형 shopdb3jo 완전 통합 복구 스크립트
--
-- ■ 생성 기준 및 반영 내역:
--   1. 원본 3조 DB 백업 (안소미디비백업_orders_260918.sql)의 모든 테이블 스키마/컬럼명 100% 엄격 준수
--   2. 신규 알람 테이블 (알람 테이블.sql -> branch_notifications) 완벽 포함
--   3. 베이스시즌 완료 보고서 (BASEASON에 맞춰 DB 바꾸기.docx) 기획 데이터 100% 반영:
--      - 의류 4대 카테고리 (OUTER, TOP, SHIRTS, PANTS)
--      - 공식 16개 패션 상품 (KS0917~KP1204) 및 실사 이미지/갤러리 매핑
--      - 64개 사이즈/컬러 옵션(Variant) 및 본사/전주/부산 지사별 실재고 배정
--      - 4대 대분류 & 14단계 세부 처리상태(process_status) 주문 라이프사이클 (#01~#14)
--      - 배송(customer_shipments), 환불/반품(refund_requests, refund_items) 데이터 완벽 동기화
--   4. 테스트 계정 비밀번호 해시를 FastAPI 백엔드와 호환되는 pbkdf2_sha256으로 설정:
--      - 관리자: admin01 / admin1234
--      - 구매자: buyer01, buyer02, buyer03, buyer96, buyer5, tester01 / buyer1234
--      - 판매자: seller01, seller02 / admin1234
-- =============================================================================
...
CREATE DATABASE IF NOT EXISTS `shopdb3jo` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE `shopdb3jo`;

SET FOREIGN_KEY_CHECKS = 0;

-- =============================================================
-- 테이블 구조 및 데이터: `org_units`
-- =============================================================
...
(4, 1, 'BR003', '베이스시즌 서울지사', 'BRANCH', '102-86-00004', '김서울', '010-9813-0820', 'seoul@baseason.co.kr', '04500', '서울특별시 중구 명동길 100', '명동빌딩 3층', 'Y', '2024-01-04 09:00:00', '2026-09-22 10:00:00'),
(5, 1, 'BR005', '베이스시즌 대구지사', 'BRANCH', '501-83-00005', '최대구', '010-8868-4457', 'daegu@baseason.co.kr', '41900', '대구광역시 중구 달구벌대로 200', '대구타워 4층', 'Y', '2024-01-05 09:00:00', '2026-09-22 10:00:00'),
(6, 1, 'BR006', '베이스시즌 광주지사', 'BRANCH', '601-84-00006', '정광주', '062-777-7777', 'gwangju@baseason.co.kr', '61900', '광주광역시 서구 상무중앙로 50', '상무빌딩 6층', 'Y', '2024-01-06 09:00:00', '2026-09-22 10:00:00');
...
(1, 1, 'admin01', '$pbkdf2-sha256$29000$TmntnXPuXct5zxkDgLB2Lg$b0mEO/ZBQuyAha7RsFWuaXQ1RZv4kc18NMAHih2ucZo', '최고관리자', 'admin@baseason.co.kr', '010-1111-1111', 'ACTIVE', '2024-01-05 10:00:00', '2026-09-22 10:00:00'),
(2, 1, 'seller01', '$pbkdf2-sha256$29000$A6DUmvN.L6U05jxHSMk5Rw$o.cBZbjBz.0kQhTjJzHxDune3i3NsQFnloYwLHWYa2E', '전주지사관리자', 'seller01@baseason.co.kr', '010-2222-2222', 'ACTIVE', '2024-02-01 10:00:00', '2026-09-22 10:00:00'),
(3, 2, 'seller02', '$pbkdf2-sha256$29000$A6DUmvN.L6U05jxHSMk5Rw$o.cBZbjBz.0kQhTjJzHxDune3i3NsQFnloYwLHWYa2E', '부산지사관리자', 'seller02@baseason.co.kr', '010-3333-3333', 'ACTIVE', '2025-01-10 10:00:00', '2026-09-22 10:00:00'),
(4, 2, 'buyer01', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', '구매자김', 'buyer01@gmail.com', '010-4444-4444', 'ACTIVE', '2024-03-01 10:00:00', '2026-09-22 10:00:00'),
(5, 3, 'buyer02', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', '이영희', 'buyer02@gmail.com', '010-5555-5555', 'ACTIVE', '2025-05-01 10:00:00', '2026-09-22 10:00:00'),
(6, 1, 'buyer03', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', '박민수', 'buyer03@gmail.com', '010-6666-6666', 'ACTIVE', '2026-01-10 10:00:00', '2026-09-22 10:00:00'),
(7, 1, 'buyer96', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', 'Buyer 96', 'buyer96@test.com', '010-9696-9696', 'ACTIVE', '2026-09-09 08:40:15', '2026-09-22 10:00:00'),
(8, 1, 'buyer5', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', '홍길동', 'test5@test.com', '010-5555-5555', 'ACTIVE', '2026-09-09 08:41:35', '2026-09-22 10:00:00'),
(10, 1, 'tester01', '$pbkdf2-sha256$29000$1nqv1ZpTCmEMAeC8V0pJ6Q$VGitwOlekv74Wi/3tNWVq0vleMxiFAWVdGSrt.sCehM', '테스터', 'tester01@example.com', '010-0000-0001', 'ACTIVE', '2026-09-17 08:22:16', '2026-09-22 10:00:00');


-- =============================================================
-- 테이블 구조 및 데이터: `user_roles`
-- =============================================================
...
(2, 1, 1, 17, 'GALLERY', '미니멀 싱글 코트 _ 브라운 상세 1', 2, 'Y', '2026-09-01 10:00:00'),
(3, 1, 1, 18, 'GALLERY', '미니멀 싱글 코트 _ 브라운 상세 2', 3, 'Y', '2026-09-01 10:00:00'),
(4, 1, 1, 19, 'GALLERY', '미니멀 싱글 코트 _ 브라운 상세 3', 4, 'Y', '2026-09-01 10:00:00'),
(5, 1, 1, 20, 'GALLERY', '미니멀 싱글 코트 _ 브라운 상세 4', 5, 'Y', '2026-09-01 10:00:00'),
(6, 1, 1, 21, 'GALLERY', '미니멀 싱글 코트 _ 브라운 상세 5', 6, 'Y', '2026-09-01 10:00:00'),
...
-- =============================================================
-- 테이블 구조 및 데이터: `branch_purchase_orders`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `branch_purchase_order_items`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `payment_transactions`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `payment_webhook_events`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `customer_shipments`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `rag_document_files`
-- =============================================================
...
-- =============================================================
-- 테이블 구조 및 데이터: `branch_notifications`
-- =============================================================
...
-- =============================================================
-- 데이터베이스 뷰 (VIEW) 정의: 3개 뷰 완벽 복구
-- =============================================================

-- 1. v_branch_purchase_order_status
DROP VIEW IF EXISTS `v_branch_purchase_order_status`;
DROP TABLE IF EXISTS `v_branch_purchase_order_status`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_branch_purchase_order_status` AS 
select 
  `bpo`.`branch_order_id` AS `branch_order_id`,
  `bpo`.`branch_order_no` AS `branch_order_no`,
  `bpo`.`org_id` AS `branch_org_id`,
  `branch`.`org_name` AS `branch_name`,
  `bpo`.`head_org_id` AS `head_org_id`,
  `head`.`org_name` AS `head_name`,
  `bpo`.`order_status` AS `order_status`,
  (case `bpo`.`order_status` 
    when 'WAITING_APPROVAL' then '지사승인대기' 
    when 'SHIPPING' then '배송중' 
    when 'RECEIVED' then '입고완료' 
    else `bpo`.`order_status`
  end) AS `order_status_name`,
  `bpoi`.`product_id` AS `product_id`,
  `p`.`product_name` AS `product_name`,
  `bpoi`.`variant_id` AS `variant_id`,
  `pv`.`sku_code` AS `sku_code`,
  `bpoi`.`order_quantity` AS `order_quantity`,
  `bpoi`.`approved_quantity` AS `approved_quantity`,
  `bpoi`.`received_quantity` AS `received_quantity`,
  `bpoi`.`unit_price` AS `unit_price`,
  `bpoi`.`item_amount` AS `item_amount`,
  `bpo`.`requested_at` AS `requested_at`,
  `bpo`.`approved_at` AS `approved_at`,
  `bpo`.`shipped_at` AS `shipped_at`,
  `bpo`.`received_at` AS `received_at` 
from (((((`branch_purchase_orders` `bpo` 
  join `org_units` `branch` on((`bpo`.`org_id` = `branch`.`org_id`))) 
  join `org_units` `head` on((`bpo`.`head_org_id` = `head`.`org_id`))) 
  join `branch_purchase_order_items` `bpoi` on((`bpo`.`branch_order_id` = `bpoi`.`branch_order_id`))) 
  join `products` `p` on((`bpoi`.`product_id` = `p`.`product_id`))) 
  join `product_variants` `pv` on((`bpoi`.`variant_id` = `pv`.`variant_id`)));

-- 2. v_customer_delivery_status
DROP VIEW IF EXISTS `v_customer_delivery_status`;
DROP TABLE IF EXISTS `v_customer_delivery_status`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_customer_delivery_status` AS 
select 
  ...
  (case 
    when `o`.`process_status` = 'PAYMENT_COMPLETED' then '결제완료' 
    when `o`.`process_status` = 'PREPARING_SHIPMENT' then '배송준비' 
    when `o`.`process_status` = 'SHIPPING' then '배송중' 
    when `o`.`process_status` = 'DELIVERED' then '배송완료' 
    when `o`.`process_status` = 'REFUND_WAITING' then '환불대기' 
    when `o`.`process_status` = 'REFUND_PROCESSING' then '환불처리중' 
    when `o`.`process_status` = 'REFUND_COMPLETED' then '환불처리완료' 
    else `o`.`process_status` 
  end) AS `process_status_name`,
  `cs`.`shipment_no` AS `shipment_no`,
  `cs`.`shipment_status` AS `shipment_status`,
  `cs`.`carrier_name` AS `carrier_name`,
  `cs`.`tracking_number` AS `tracking_number`,
  `o`.`total_amount` AS `total_amount`,
  `o`.`ordered_at` AS `ordered_at`,
  `cs`.`shipped_at` AS `shipped_at`,
  `cs`.`delivered_at` AS `delivered_at` 
from (((`orders` `o` 
  join `org_units` `ou` on((`o`.`org_id` = `ou`.`org_id`))) 
  join `users` `u` on((`o`.`buyer_user_id` = `u`.`user_id`))) 
  left join `customer_shipments` `cs` on((`o`.`order_id` = `cs`.`order_id`)));

-- 3. v_hq_inventory_status
DROP VIEW IF EXISTS `v_hq_inventory_status`;
DROP TABLE IF EXISTS `v_hq_inventory_status`;
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `v_hq_inventory_status` AS 
select 
  `hi`.`hg_inventory_id` AS `hg_inventory_id`,
  `hi`.`org_id` AS `org_id`,
  `ou`.`org_name` AS `org_name`,
  ...
  `pv`.`sku_code` AS `sku_code`,
  `pv`.`option_name1` AS `option_name1`,
  `pv`.`option_value1` AS `option_value1`,
  `pv`.`option_name2` AS `option_name2`,
  `pv`.`option_value2` AS `option_value2`,
  `hi`.`stock_quantity` AS `stock_quantity`,
  `hi`.`reserved_quantity` AS `reserved_quantity`,
  `hi`.`safety_stock` AS `safety_stock`,
  (`hi`.`stock_quantity` - `hi`.`reserved_quantity`) AS `available_quantity`,
  `hi`.`updated_at` AS `updated_at` 
from (((`hg_inventory` `hi` 
  join `org_units` `ou` on((`hi`.`org_id` = `ou`.`org_id`))) 
  join `products` `p` on((`hi`.`product_id` = `p`.`product_id`))) 
  join `product_variants` `pv` on((`hi`.`variant_id` = `pv`.`variant_id`)));
