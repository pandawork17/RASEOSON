...
export function LoginPage({ onSuccess, onGoRegister }) {
  ...
  const [customAccounts, setCustomAccounts] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("registered_demo_accounts") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const doLogin = async (id, pw) => {
    ...
    try {
      ...
      setError(msg);
      window.alert(`[로그인 실패]\n${msg}\n\n※ 해당 계정이 DB에 존재하지 않거나 비밀번호가 다를 수 있습니다. (목록에서 삭제하려면 카드 우측의 '×' 버튼을 클릭해주세요)`);
    } finally {
      setSubmitting(false);
  ...
  const handleRemoveCustomAccount = async (e, targetLoginId) => {
    e.stopPropagation();
    if (!window.confirm(`'${targetLoginId}' 계정을 데이터베이스(DB) 및 시연 목록에서 완전히 삭제하시겠습니까?`)) {
      return;
    }
    try {
      await api.deleteUser(targetLoginId);
      window.alert("계정이 데이터베이스(DB)에서 성공적으로 삭제되었습니다.");
    } catch (err) {
      console.warn("DB 사용자 삭제 실패 또는 이미 삭제됨:", err);
    ...
    try {
      ...
      if (localStorage.getItem("last_login_id") === targetLoginId) {
  ...
  return (
    <main className="auth-page">
      <div className="auth-card figma-auth-card">
              ...
              >
                <div className="demo-acc-top">
                  <div className="demo-acc-title">
                    <strong className="acc-name">{acc.nickname || acc.user_name}</strong>
        ...
        {/* 시연을 위해 직접 아이디/비밀번호 입력 영역 주석 처리 */}
        {/*
