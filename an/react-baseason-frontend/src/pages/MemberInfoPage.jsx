...
export function MemberInfoPage({
  ...
  useEffect(() => {
    if (authUser) {
      ...
      if (authUser.phone) {
        ...
        if (raw.length === 11) {
          setPhonePrefix(raw.slice(0, 3));
          setPhoneMid(raw.slice(3, 7));
          setPhoneEnd(raw.slice(7, 11));
        } else if (raw.length === 10) {
          if (raw.startsWith("02")) {
            setPhonePrefix("02");
            setPhoneMid(raw.slice(2, 6));
            setPhoneEnd(raw.slice(6, 10));
          } else {
            setPhonePrefix(raw.slice(0, 3));
            setPhoneMid(raw.slice(3, 6));
            setPhoneEnd(raw.slice(6, 10));
          }
        } else {
          ...
          if (parts.length === 3) {
            ...
            setPhonePrefix("010");
            setPhoneMid(raw.slice(0, 4));
            setPhoneEnd(raw.slice(4, 8));
          } else {
            setPhonePrefix("010");
            setPhoneMid(raw);
            setPhoneEnd("");
      ...
      if (authUser.user_id) {
        try {
          const saved = localStorage.getItem(`marketing_agree_${authUser.user_id}`);
          if (saved !== null) {
            const parsed = JSON.parse(saved);
            setAgreeEmail(Boolean(parsed.email));
            setAgreeSms(Boolean(parsed.sms));
          } else {
            setAgreeEmail(false);
            setAgreeSms(false);
          }
  ...
  const handleSave = async (e) => {
    ...
    const fullPhone = phoneMid.trim() && phoneEnd.trim() ? `${phonePrefix}-${phoneMid.trim()}-${phoneEnd.trim()}` : "";
    const selectedDomain = emailDomain === "custom" ? customDomain.trim() : emailDomain;
    const fullEmail = emailUser.trim() && selectedDomain ? `${emailUser.trim()}@${selectedDomain}` : "";
      ...
      if (authUser?.user_id) {
        try {
          localStorage.setItem(
            `marketing_agree_${authUser.user_id}`,
            JSON.stringify({ email: agreeEmail, sms: agreeSms })
          );
        } catch (err) {
          console.error("마케팅 수신 동의 저장 실패:", err);
        }
      }
